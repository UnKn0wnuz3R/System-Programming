...
/*
Creates and returns an expanded copy of bitmap 
with size additional bits, copying the original bits*/
struct bitmap *bitmap_expand(struct bitmap *bitmap,int size)
{
  struct bitmap *elem_ptr = bitmap_create(bitmap_size(bitmap)+size);

  for(int idx=0;idx<bitmap_size(elem_ptr);idx++)
  {
    if(idx >= bitmap_size(bitmap))
      break;

    if(bitmap_test(bitmap,idx))
    {
      bitmap_mark(elem_ptr,idx);
    }
  }
